...
void MarkovBrain::update() {
    ...
    for (int eval = 0; eval < evaluationsPreUpdate; eval++) {
        ...
        //std::cout << "  collecting t_state..." << std::endl;
        /*
        t_state.clear();
        for (auto e : nodes) {
            t_state.push_back((e));
        ...
        */
        ...
        if (recurrentOutput) {
            ...
            for (int h = nrInputValues + nrOutputValues; h < nodes.size(); h++) {// copy only hidden
                nodes[h] = nextNodes[h];
            }
        }
        ...
        if (recordIOMapPL->get()) {
            ...
            for (int i = 0; i < nrOutputValues; i++) {
                IOMap.append("output", Bit(nextNodes[nrInputValues + i]));
            }

            for (int i = nrInputValues + nrOutputValues; i < nodes.size(); i++) {
                IOMap.append("hidden", Bit(nextNodes[i]));
            }


            
            //std::cout << "  collecting tt_state..." << std::endl;
            /*
            tt_state.clear();

            for (auto e : nextNodes) {
                tt_state.push_back((e));
            };
            ...
            for (int q = 0; q < tt_state.size(); q++) {
                if (checkDet[q].find(t_state) != checkDet[q].end()) {
                    std::cout << "  found t_state in map..." << std::endl;

                    if (checkDet[q][t_state] != tt_state[q]) {
                        std::cout << "found a problem...\n   both " <<  checkDet[q][t_state]  << " and " << tt_state[q] << " resulted from ";
                        for (auto e : t_state) {
                            std::cout << e << " ";
                        }
                        std::cout << std::endl;
